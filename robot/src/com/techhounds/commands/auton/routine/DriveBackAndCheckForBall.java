...
public class DriveBackAndCheckForBall extends CommandGroup {
	...
	public DriveBackAndCheckForBall() {
		addParallel(drive = new DriveDistance(-85, -AutoMap.LOW_BAR_SPEED, -AutoMap.LOW_BAR_SPEED + .1, 3));
		addSequential(ballCheck = new WaitForBeanBreak(true));
	}
	public DriveBackAndCheckForBall(double stopDistance){
		addParallel(drive = new DriveDistanceStraight(stopDistance, -AutoMap.LOW_BAR_SPEED + .1, -RobotMap.DriveTrain.MIN_STRAIGHT_POWER, 5.0, true, 0.25));
		addSequential(ballCheck = new WaitForBeanBreak(true));
	}
